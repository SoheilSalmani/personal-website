...
1. Create a Java program that declares 7 different types of variables, and print them. Build the project and run it from the CLI.
...
6. Create the _WordTool_ application (methods: `countWords`, `countSymbols`, `getCharFrequency`).
...
4. Write a `prettyPrintApple` method that takes a `List` of `Apple`s and that can be
